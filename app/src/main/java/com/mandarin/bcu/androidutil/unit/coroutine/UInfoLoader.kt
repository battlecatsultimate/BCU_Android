...
class UInfoLoader(activity: Activity, private val data: Identifier<Unit>, private val fm: FragmentManager) : CoroutineTask<String>() {
    ...
    override fun finish() {
        ...
        if (activity.resources.configuration.orientation == Configuration.ORIENTATION_LANDSCAPE) {
            if (preferences.getBoolean("Lay_Land", false)) {
                ...
                val table = activity.findViewById<MeasureViewPager>(R.id.unitinftable)
