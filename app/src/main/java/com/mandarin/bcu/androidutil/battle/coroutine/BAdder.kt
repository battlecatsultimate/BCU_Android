                        ...
                        SoundHandler.MUSIC.setOnPreparedListener(object : MediaPrepare() {
                            override fun prepare(mp: MediaPlayer?) {
                                if (SoundHandler.musicPlay) {
                                    if((battleView.painter.bf.sb.st.mus0.get()?.loop ?: 0) > 0) {
                                        if((battleView.painter.bf.sb.st.mus0.get()?.loop ?: 0) < SoundHandler.MUSIC.duration) {
                                            SoundHandler.timer = object : PauseCountDown((SoundHandler.MUSIC.duration-1).toLong(), (SoundHandler.MUSIC.duration-1).toLong(), true) {
                                                override fun onFinish() {
                                                    SoundHandler.MUSIC.seekTo((battleView.painter.bf.sb.st.mus0.get()?.loop ?: 0).toInt(), true)

                                                    SoundHandler.timer = object : PauseCountDown((SoundHandler.MUSIC.duration-1).toLong()-(battleView.painter.bf.sb.st.mus0.get()?.loop ?: 0), (SoundHandler.MUSIC.duration-1).toLong()-(battleView.painter.bf.sb.st.mus0.get()?.loop ?: 0), true) {
                                                        override fun onFinish() {
                                                            SoundHandler.MUSIC.seekTo((battleView.painter.bf.sb.st.mus0.get()?.loop ?: 0).toInt(), true)
