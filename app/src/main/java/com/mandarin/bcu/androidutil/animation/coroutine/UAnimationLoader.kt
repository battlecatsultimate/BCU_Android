...
open class UAnimationLoader(activity: Activity, private val data: Identifier<Unit>?, private val form: Int) : CoroutineTask<String>() {
    companion object {
    ...
    override fun progressUpdate(vararg data: String) {
        ...
        when (data[0]) {
            ...
            PROCESS -> {
                ...
                val cView = AnimationCView(activity, this.data, StaticStore.formposition, 0, !shared.getBoolean("theme", false), shared.getBoolean("Axis", true), frame, controller, fps, gif)
                ...
                while (i < u.forms.size) {
                    val n = if (this.data.pack == Identifier.DEF) {
                        "Default-${this.data.id}-${StaticStore.trio(i)}"
                    } else {
                        "${this.data.pack}-${this.data.id}-${StaticStore.trio(i)}"
                    }
                    ids.add(n)
                    i++
                }
                    ...
                    when (item.itemId) {
                        ...
                        R.id.anim_png_normal -> {
                            ...
                            val name2 = if (this.data.pack == Identifier.DEF) {
                                dateFormat.format(date) + "-U-" + "Default" + "-" + StaticStore.trio(this.data.id) + "-" + form
                            } else {
                                "${dateFormat.format(date)}-U-${this.data.pack}-${StaticStore.trio(this.data.id)}-$form"
                            }
                        ...
                        R.id.anim_png_transp -> {
                            ...
                            val dateFormat = SimpleDateFormat("yyyy_MM_dd_HH_mm_ss", Locale.US)
                            val date = Date()
                            val name1 = if (this.data.pack == Identifier.DEF) {
                                dateFormat.format(date) + "-U-Trans-" + "Default" + "-" + StaticStore.trio(this.data.id) + "-" + form
                            } else {
                                "${dateFormat.format(date)}-U-Trans-${this.data.pack}-${StaticStore.trio(this.data.id)}-$form"
                            }
                        ...
                        R.id.anim_gif_range -> {
                            if(!StaticStore.gifisSaving) {
                                ...
                                val gifAdapter = GIFRangeRecycle(name, activity, AnimationCView.UNIT, this.data, StaticStore.formposition)
                                ...
                                gen.setOnClickListener(object : SingleClick() {
                                    override fun onSingleClick(v: View?) {
                                        ...
                                        GIFRange(activity, this@UAnimationLoader.data, StaticStore.formposition, cView, !shared.getBoolean("theme", false), gifAdapter.getData(), gifAdapter.getEnables()).execute()
