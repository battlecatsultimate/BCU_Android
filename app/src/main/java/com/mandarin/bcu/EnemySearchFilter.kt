...
open class EnemySearchFilter : AppCompatActivity() {
    private val traits = arrayOfNulls<CheckBox>(12)
    private val attacks = arrayOfNulls<CheckBox>(3)
    private val abilities = arrayOfNulls<CheckBox>(20)
    ...
    private val abid = intArrayOf(R.id.eschchabwe, R.id.eschchabfr, R.id.eschchabsl, R.id.eschchabkb, R.id.eschchabwp, R.id.eschchabstr, R.id.eschchabsu, R.id.eschchabcd, R.id.eschchabcr, R.id.eschchabwv, R.id.eschchabimwe,
    ...
    private val abils = arrayOf(intArrayOf(1, Data.P_WEAK), intArrayOf(1, Data.P_STOP), intArrayOf(1, Data.P_SLOW), intArrayOf(1, Data.P_KB), intArrayOf(1, Data.P_WARP), intArrayOf(1, Data.P_STRONG), intArrayOf(1, Data.P_LETHAL), intArrayOf(0, Data.AB_BASE), intArrayOf(1, Data.P_CRIT), intArrayOf(1, Data.P_WAVE), intArrayOf(1, Data.P_IMUWEAK), intArrayOf(1, Data.P_IMUSTOP), intArrayOf(1, Data.P_IMUSLOW), intArrayOf(1, Data.P_IMUKB), intArrayOf(1, Data.P_IMUWAVE), intArrayOf(1, Data.P_CURSE), intArrayOf(1, Data.P_BURROW), intArrayOf(1, Data.P_REVIVE), intArrayOf(1, Data.P_SATK), intArrayOf(1, Data.P_POIATK))
    private val atkdraw = intArrayOf(212, 112)
    private val trdraw = intArrayOf(219, 220, 221, 222, 223, 224, 225, 226, 227, -1, -1, -1)
    private val abdraw = intArrayOf(195, 197, 198, 207, 266, 196, 199, 200, 201, 208, 213, 214, 215, 216, 210, -1, -1, -1, 229, -1)
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        for (i in trid.indices) {
            ...
            if (resources.configuration.orientation == Configuration.ORIENTATION_LANDSCAPE && trdraw[i] != -1) traits[i]?.setCompoundDrawablesWithIntrinsicBounds(null, null, getResizeDraw(trdraw[i], 40f), null) else if (trdraw[i] != -1) traits[i]?.setCompoundDrawablesWithIntrinsicBounds(null, null, getResizeDraw(trdraw[i], 32f), null)
            if (trdraw[i] != -1) traits[i]?.compoundDrawablePadding = StaticStore.dptopx(16f, this)
        }
        ...
        for (i in abid.indices) {
            ...
            if (resources.configuration.orientation == Configuration.ORIENTATION_LANDSCAPE) if (abdraw[i] != -1) {
                if (abdraw[i] == -100) {
                ...
                val b = StaticStore.getResizeb(BitmapFactory.decodeFile(Environment.getExternalStorageDirectory().path + "/Android/data/com.mandarin.BCU/files/org/page/icons/" + abfiles[i]), this, 40f)
                ...
                val b = StaticStore.getResizeb(BitmapFactory.decodeFile(Environment.getExternalStorageDirectory().path + "/Android/data/com.mandarin.BCU/files/org/page/icons/" + abfiles[i]), this, 32f)
                abilities[i]?.setCompoundDrawablesWithIntrinsicBounds(null, null, BitmapDrawable(resources, b), null)
            }
    ...
    protected fun listeners() {
        ...
        reset!!.setOnClickListener {
            ...
            for (trait in traits) {
                if (trait!!.isChecked) trait.isChecked = false
            }
            for (ability in abilities) if (ability!!.isChecked) ability.isChecked = false
        }
        ...
        for (i in traits.indices) {
            traits[i]!!.setOnCheckedChangeListener { _, isChecked -> if (isChecked) StaticStore.tg.add(colors[i]) else StaticStore.tg.remove(colors[i]) }
        ...
        for (i in abilities.indices) {
            ...
            abilities[i]!!.setOnCheckedChangeListener { _, isChecked ->
                ...
                if (isChecked) StaticStore.ability.add(abilval) else StaticStore.ability.remove(abilval)
            }
            star.setOnCheckedChangeListener { _, isChecked -> StaticStore.starred = isChecked }
        }
    }
    ...
    private fun checker() {
        ...
        if (!StaticStore.atksimu) if (!StaticStore.empty) atkgroup!!.check(R.id.eschrdatksi)
        if (!StaticStore.atkorand) atkgroupor!!.check(R.id.eschrdatkand)
        if (!StaticStore.tgorand) tggroup!!.check(R.id.eschrdtgand)
        if (!StaticStore.aborand) abgroup!!.check(R.id.eschrdaband)
        for (i in atks.indices) {
            if (StaticStore.attack != null && StaticStore.attack.contains(atks[i])) attacks[i]!!.isChecked = true
        }
        for (i in colors.indices) {
            if (StaticStore.tg != null && StaticStore.tg.contains(colors[i])) traits[i]!!.isChecked = true
        }
        for (i in abils.indices) {
            val checker = ArrayList<Int>()
            for (k in abils[i]) checker.add(k)
            if (StaticStore.ability != null && StaticStore.ability.contains(checker)) abilities[i]!!.isChecked = true
        }
    }
